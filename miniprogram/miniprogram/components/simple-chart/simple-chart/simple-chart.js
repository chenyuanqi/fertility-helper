    ...
    drawDataPoints(ctx, padding, chartWidth, chartHeight, minTemp, maxTemp) {
      ...
      this.data.chartData.forEach((day, index) => {
        ...
        if (day.temperature) {
          ...
          // 体温标签
          ctx.fillStyle = '#DC143C'; // 深红色，确保文字清晰
        ...
        if (this.data.viewMode === 'all') {
          // 月经标记 - 在体温点左侧垂直排列
          if (day.menstrual) {
            const menstrualX = x - 12;
            const menstrualY = day.temperature ? 
              (padding.top + chartHeight - ((day.temperature - minTemp) / tempRange) * chartHeight) :
              (padding.top + chartHeight * 0.8);
            ...
            ctx.fillStyle = colors[day.menstrual] || '#4169E1';
            ctx.beginPath();
            ctx.arc(menstrualX, menstrualY, 5, 0, 2 * Math.PI);
            ctx.fill();
            ...
            ctx.textAlign = 'center';
            ctx.fillText('M', menstrualX, menstrualY + 3);
          }
          
          // 同房标记 - 在体温点右侧垂直排列
          if (day.intercourse) {
            const intercourseX = x + 12;
            const intercourseY = day.temperature ? 
              (padding.top + chartHeight - ((day.temperature - minTemp) / tempRange) * chartHeight) :
              (padding.top + chartHeight * 0.8);
            
            ctx.fillStyle = '#32CD32'; // 绿色
            ctx.beginPath();
            ctx.arc(intercourseX, intercourseY, 5, 0, 2 * Math.PI);
            ctx.fill();
            ...
            if (day.intercourse > 1) {
              ctx.fillText(day.intercourse.toString(), intercourseX, intercourseY + 3);
            } else {
              ctx.fillText('♥', intercourseX, intercourseY + 3);
            }
          }
        }
      });
